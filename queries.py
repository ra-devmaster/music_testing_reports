...
def create_message_queue_entries(conn):
    ...
    try:
        q = ('SELECT report_id, radio_ids, demo_id FROM radioanalyzer.music_testing_reports AS mts JOIN radioanalyzer.users AS u USING(user_id) WHERE being_processed IN (1, -1'
             ') AND ('
             'DATE_ADD('
